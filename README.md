...
2. **Get HubSpot cookies**: log into `app-eu1.hubspot.com` → DevTools → Application → Cookies → `app-eu1.hubspot.com` → copy the full cookie string and `hubspotapi-csrf` value.
...
4. **Schedule daily** (cron):
   ```cron
   0 6 * * * /full/path/to/hubspot-beta-tracker/scripts/hubspot-beta-scan.sh >> /var/log/beta-scan.log 2>&1
   ```

> **Cookie expiry**: HubSpot session cookies expire — when descriptions stop updating, refresh the cookies from your browser. Use `./rotate-auth.sh` to update Railway quickly.
